...
WITH user_level_counts AS (
  -- Find all users who completed at least 1000 levels starting from level 1
  SELECT
    user_pseudo_id,
    MIN((SELECT value.int_value FROM UNNEST(event_params) WHERE key = 'level')) as first_level,
    MAX((SELECT value.int_value FROM UNNEST(event_params) WHERE key = 'level')) as max_level
  ...
  HAVING first_level = 1 AND max_level >= 1000
),
random_user AS (
  -- Select one random user from those who completed 1000+ levels starting from level 1
...
all_completions AS (
  ...
  WHERE event_name = 'level_complete'
    ...
    AND (SELECT value.int_value FROM UNNEST(event_params) WHERE key = 'level') BETWEEN 1 AND 1000
),
battle_loadouts AS (
...
-- Show completions for levels 1-1000 with detailed timing and unit loadouts
SELECT
  user_pseudo_id,
  completion_number,
  level,
